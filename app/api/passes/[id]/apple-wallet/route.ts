...
import {
  generateApplePassPreview,
  generateApplePassPkpass,
...
export async function GET(_req: Request, { params }: RouteParams) {
  try {
    ...
    const qrUrl = await qrDataUrl(pass.code);
    ...
    const previewInput = {
      ...
      coverImageUrl: pass.events.cover_image_url ?? undefined,
      qrDataUrl: qrUrl,
    };

    const preview = generateApplePassPreview(previewInput);

    const pkpass = await generateApplePassPkpass(previewInput);
      ...
      return NextResponse.json(
        {
          ...
          preview,
          message:
            "Apple Wallet certs not configured — add APPLE_WALLET_CERT env var",
        },
        { status: 202 },
      );
    }
    ...
    return new Response(new Uint8Array(pkpass), {
      ...
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="wefetepass.pkpass"',
        "Content-Length": String(pkpass.length),
      },
    });
  } catch (err) {
    console.error("[GET /api/passes/[id]/apple-wallet]", err);
    return NextResponse.json(
